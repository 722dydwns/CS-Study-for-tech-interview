...
![Untitled](https://prod-files-secure.s3.us-west-2.amazonaws.com/e2aaace0-24ef-4ae8-bed4-d8cb2e34acd9/a325527a-b6f1-4e9f-a7ec-b82bdf6f7715/Untitled.png)

![Untitled](https://prod-files-secure.s3.us-west-2.amazonaws.com/e2aaace0-24ef-4ae8-bed4-d8cb2e34acd9/02ac2990-e4c9-478a-b9e3-86955373022b/Untitled.png)

쿠키 빠르다. 보안 취약하다. → 세션 보안은 낫다. 동시 접속자 많아지면 세션 ID 모두 세션 저장소에 기록해야 되니까 서버 부하 심하다 → 토큰은 서버 부담 줄이고자 클라이언트가 직접 토큰을 들고 있는 방식이다.  
...
**b) 지속 쿠키 ( Persistent Cookie )** : 유효시간을 정하지 않은 경우 브라우저가 종료되어도 남아 있는 쿠키
...
- 세션의 대안으로 등장 (서버 부담 줄이기 위해 클라이언트 측에 상태 저장)

**[세션 인증 vs 토큰 인증]**

- **세션 인증 : 서버가 세션 id 저장 후 클라이언트가 실어보낸 쿠키 속 세션 id와 대조해서 확인함**
- **토큰 인증 : 해당 토큰 유효한지만 확인함 (효율 더 좋음)**
- JWT 는 발급한 후 검증만 하기 때문에 추가 저장소가 필요하지 않습니다.
- (StateLess - 상태/정보 저장하지 않음) 이는 서버를 확장하거나 유지, 보수하는데 유리합니다.

**3) 저장 위치 : 클라이언트 측에 저장한 것**

- 세션이 서버에 저장된다면, 토큰은 클 측에 저장한다
...
- 쿠키 대체할 목적으로 HTML5 부터 등장한 key-value 데이터
...
- 쿠키와 비슷하나 목적성에 차이가 있다.

쿠키 : 사용자를 위함 (세션 관리, 개인화(user에 맞는 페이지 )

캐시 : 반복 사용 데이터를 임시저장해두고 재접속 시 서버 부담 줄이고 로딩 시간 단축시킴
