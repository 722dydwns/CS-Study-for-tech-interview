...
- **보통 두 테이블 직접 연결 X. 각 테이블의 PK를 FK로 참조하고 있는 (매 테이블) 사용함**
- **즉, 양쪽 두 테이블과 1:N 관계형성하는 새로운 테이블로 N:M 관계를 나타낸다.**
...
- 릴레이션에서 특정 튜플 식별 시 사용하는 속성 집합이다.
- **각 튜플 유일 식별 장치**
- **릴레이션 간의 관계 알려주는 장치**
...
- **두 개 이상의 테이블에서 조건 만족하는 일부 데이터만 얻는 쿼리**
...
**→ 일반적으로 여러 테이블 연관 시킬 때, 데이터 대량인 경우 데이터를 모두 합치는 조인 보다는 필요한 데이터만 찾아서 공급해주는 부속질의 성능이 더 좋다.**
