...
다른 트랜잭션이 이미 점유하고 있는 데이터를 빼앗거나 대기하는 방식 
...
## 1) Transaction 동시 실행 문제 (3)

편의 상 T1 (읽기), T2 (쓰기) 하는 트랜잭션이라 가정한다.

### 1) 오손 읽기 | Dirty Read

**T2가 쓴 작업이 (커밋 되기 전에) Rollback되어 T1이 읽은 데이터가 무효화 된 문제**
...
**(3) 다른 트랜잭션이 공유락 걸린 데이터에 UPDATE는 못하지만, 나머지 범위에 INSERT는 가능하다.**   → 그래서 여전히 유령 데이터 읽는 문제는 발생함
